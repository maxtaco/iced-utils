...
exports.Runner = class Runner
  ...
  get_fn_name : (f) ->
    if (m = f.toString().match /function (\S+) \{/) then m[1]
    else "anonymous"
  ...
  get_test_order : (code) ->
    ...
    if code.order?
      for k,v of code
        if k is order
          for f in v
            out.push [ @get_fn_name(v), v]
        else
          out.push [k,v]
    else
      out.push [k,v] for k,v of code
    return out
  ...
  run_code : (fn, code, cb) ->
    ...
    # We're going to try to do things in order, if order is important for
    order = @get_test_order code
